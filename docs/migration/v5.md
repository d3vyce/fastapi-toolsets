...
### Lifecycle events need `session_class=EventSession`

Because `Database` now builds the session factory, the `class_=EventSession` you used to pass to your own `async_sessionmaker` applies to a factory your app no longer uses. Move it to the `session_class` argument of `Database`, or [lifecycle events](../module/models.md#lifecycle-events) **stop firing silently**: nothing raises and nothing is logged.
